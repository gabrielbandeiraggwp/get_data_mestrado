...
adf_inflacao <- ur.df(desvio_inflacao_dataframe$desvios, type =c("drift"), selectlags = "BIC")
...
adf_hiato <- ur.df(hiato$hiato, type =c("drift"), selectlags = "BIC")
...
adf_cambio <- ur.df(CAMBIO$value, type =c("trend"), selectlags = "BIC")
...
adf_EMBI <- ur.df(EMBI$EMBI_media, type =c("drift"), selectlags = "BIC")
...
adf_divida <- ur.df(DIVIDA_LIQUIDA$DIVIDA_LIQUIDA, type =c("trend"), selectlags = "BIC")
...
#teste adf e kpss para a Selic (t)
ggplot(data= SELIC, mapping = aes(x = ref.date, y = SELIC))+
  geom_line(color = "turquoise4") +
  labs(title = "selica: 2003 - 2024", y = "selic")
adf_selic_c <- ur.df(SELIC$SELIC, type =c("trend"), selectlags = "BIC")
kpss_selic_c <- ur.kpss(SELIC$SELIC, type = "tau") #obs^o tau é equvalente a "trend"
pp_selic_c <- ur.pp(SELIC$SELIC, 
                    type = "Z-tau",      
                    model = "trend")
summary(adf_selic_c) #
summary(kpss_selic_c)
summary(pp_selic_c)
sd(SELIC$SELIC)
acf(SELIC$SELIC, plot = TRUE)
pacf(SELIC$SELIC, plot = TRUE)
...
ggplot(data= SELIC, mapping = aes(x = ref.date, y = SELIC_defasado))+
  geom_line(color = "turquoise4") +
  labs(title = "selica: 2003 - 2024", y = "selic")
adf_selic <- ur.df(SELIC$SELIC_defasado, type =c("trend"), selectlags = "BIC")
...
adf_COMODITIES <- ur.df(COMODITIES$value, type =c("trend"), selectlags = "BIC")
